...
struct CompoundButtonView: View {
    ...
    private static func resolveFluentIcon(_ name: String) -> String {
        ...
        // If baseName is already a valid SF Symbol name, use it directly
        // This handles card JSON that uses SF Symbol names like "checkmark.circle.fill", "star.fill", "trash"
        if baseName.contains(".") || sfSymbolDirectNames.contains(baseName) {
            return baseName
        }
